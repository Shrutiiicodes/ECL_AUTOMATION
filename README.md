...
├── docs/
│   └── refactor.md      ← Notes on the pipeline refactor
├── data/                ← Git-ignored intermediate data and sqlite db
├── output/              ← Git-ignored output Excel deliverables
├── legacy/              ← Legacy scripts and reference material
├── requirements.txt     ← numpy, pandas, python-dateutil, openpyxl
└── .gitignore           ← Ignores data/, output/, legacy/, *.csv, *.xlsx
```

Everything in `data/` and `output/` is regenerated by `python main.py` and git-ignored. The `legacy/` directory (screenshots + legacy scripts) is also git-ignored.
